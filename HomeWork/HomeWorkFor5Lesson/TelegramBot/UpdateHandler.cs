    ...
    class UpdateHandler : IUpdateHandler
    {
            ...
            try
            {
                ...
                switch (update.Message.Text.Split(' ')[0])
                {
                    ...
                    default:
                        ...
                        if (user != null)
                            await botClient.SendMessage(update.Message.Chat, $"{user.TelegramUserName}, вы ввели неизвестную команду:", ct);
                        else
        ...
        private async Task FindTask(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            else
            {
                ...
                {
                    await botClient.SendMessage(update.Message.Chat, $"{item.Name} - {item.CreatedAt} - {item.Id}", ct);
                }
            }
        }
        ...
        private async Task ReportStats(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            await botClient.SendMessage(update.Message.Chat, $"Статистика по задачам на {stats.generatedAt}. Всего: {stats.total}; Завершенных: {stats.completed}; Активных: {stats.active};", ct);
        }
        ...
        private async Task ShowAllTasks(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            await botClient.SendMessage(update.Message.Chat, "Вот ваш список задач:", ct);
        ...
        private async Task CompleteTask(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            await toDoService.MarkCompleted(delGuig, ct);
        }
        ...
        private async Task EnableCommand(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            var user = await userService.GetUser(update.Message.From.Id, ct);
            if (user != null)
                await botClient.SendMessage(update.Message.Chat, $"{user.TelegramUserName}, вам доступные команды: /start, /help/, /info, /addttask, /showtasks, /removetask, /completetask, /showalltasks ,/report, /find, /exit", ct);
            else
                await botClient.SendMessage(update.Message.Chat, @"Доступные команды: /start, /help/, /info, /exit", ct);
        }
        ...
        private async Task HelpCommand(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            var user = await userService.GetUser(update.Message.From.Id, ct);
            if (user != null)
                await botClient.SendMessage(update.Message.Chat, $"{user.TelegramUserName}, вам доступные команды:", ct);
            else
                await botClient.SendMessage(update.Message.Chat, "Вам доступные команды:", ct);
            if (user != null)
            {
                await botClient.SendMessage(update.Message.Chat, @"  /addtask : позволяет добавить задачу в список.", ct);
                await botClient.SendMessage(update.Message.Chat, @"  /showtasks : выводит список активных задач.", ct);
                await botClient.SendMessage(update.Message.Chat, @"  /removetask : позвояет удаить задачу.", ct);
                await botClient.SendMessage(update.Message.Chat, @"  /completetask : заверршает задачу.", ct);
                await botClient.SendMessage(update.Message.Chat, @"  /showalltasks : выводит все задачи.", ct);
                await botClient.SendMessage(update.Message.Chat, @"  /report : выводит статистику по задачам.", ct);
                await botClient.SendMessage(update.Message.Chat, @"  /find : выводит все задачи которые начинаются с заданного значения.", ct);
            }
            await botClient.SendMessage(update.Message.Chat, @"  /start : позволяет ввести в порграмму Ваше имя, что бы прорграмма Вас узнавала.", ct);
            await botClient.SendMessage(update.Message.Chat, @"  /help : отображает краткую справочную информацию о том, как пользоваться программой.", ct);
            await botClient.SendMessage(update.Message.Chat, @"  /info : предоставляет информацию о версии программы и дате её создания.", ct);
            await botClient.SendMessage(update.Message.Chat, @"  /exit : завершает программу.", ct);
        }
        ...
        private async Task InfoCommand(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            var user = await userService.GetUser(update.Message.From.Id, ct);
            if (user != null)
                await botClient.SendMessage(update.Message.Chat, $"{user.TelegramUserName}, предоставляю Вам информацию о программе:", ct);
            else
                await botClient.SendMessage(update.Message.Chat, "Информация о программе:", ct);
            await botClient.SendMessage(update.Message.Chat, $"  Версия: {assembly.GetName().Version}", ct);
            await botClient.SendMessage(update.Message.Chat, $"  Дата создания: {File.GetLastWriteTime(assembly.Location)}", ct);
        }
        ...
        private async Task AddTaskCommand(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            if (string.IsNullOrEmpty(str))
                await botClient.SendMessage(update.Message.Chat, "Нельзя добавить задачу с пустым описанием", ct);
            else
            {
                var newTask = toDoService.Add(user, str, ct);
                await botClient.SendMessage(update.Message.Chat, $@"Задача ""{str}"" добавлена в список", ct);
            }
        }
        ...
        private async Task RemoveTaskCommand(ITelegramBotClient botClient, Update update, CancellationToken ct)
        {
            ...
            Guid delGuig = new Guid(str);
            await toDoService.Delete(delGuig, ct);
        }
    }
}
